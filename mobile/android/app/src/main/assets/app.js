...
async function translateRead() {
  ...
  const oe = $("#readOut"), de = $("#readDir"), cb = $("#readCopy"); oe.classList.remove("placeholder", "err"); oe.classList.add("typing"); oe.textContent = "";
  ...
  try {
    ...
    if (parseErrors > 0) toast("部分数据解析异常");
    oe.classList.remove("typing"); cb.style.display = ""; if (!full.trim()) { oe.classList.add("err"); oe.textContent = "无返回内容"; }
...
if (window.visualViewport) {
  let _vpRaf = 0;
  const handleViewport = () => { cancelAnimationFrame(_vpRaf); _vpRaf = requestAnimationFrame(() => { const vh = window.visualViewport.height; document.body.style.height = vh + "px"; const isMobile = window.matchMedia("(max-width:640px)").matches; const offset = isMobile ? Math.max(0, window.innerHeight - vh) : 0; const c = document.getElementById("ctxSheet"), w = document.getElementById("wizard"); if (c) c.style.paddingBottom = offset + "px"; if (w) w.style.paddingBottom = offset + "px"; }); };
